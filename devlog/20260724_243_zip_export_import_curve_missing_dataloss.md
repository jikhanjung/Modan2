# ZIP 데이터셋 export/import의 데이터 손실 — 세미랜드마크 커브 소실 + missing 인코딩
...
`MdUtils.import_dataset_from_zip`(23→<15)을 정리한 직후, "zip export/import가
missing·curve 데이터를 제대로 처리하는가?"라는 물음을 실제 라운드트립으로
검증했다. **두 개의 데이터 손실 버그**가 드러났다.
...
`test_zip_roundtrip_preserves_missing_landmarks_and_curves` 추가 — 커브 스킴·raw·
anchors 보존과 missing 랜드마크(그 `"Missing"` 인코딩)를 단언. 기존 1.1-입력 import
테스트는 그대로 통과하여 하위호환을 검증한다.

## 교훈

- **직렬화 스키마는 기능이 늘 때 함께 커야 한다.** 세미랜드마크 커브(devlog
  237~240)가 추가됐지만 export/import 매니페스트는 그에 맞춰 확장되지 않았다.
  이런 누락은 UI 테스트로는 안 잡히고 **라운드트립 테스트**로만 드러난다.
