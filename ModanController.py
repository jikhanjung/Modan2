...
class ModanController(QObject):
    ...
    def create_dataset(
        self, name: str, desc: str, dimension: int, landmark_count: int, **kwargs
    ) -> MdModel.MdDataset | None:
        ...
        try:
            ...
            existing = MdModel.MdDataset.select().where(MdModel.MdDataset.dataset_name == name)
            if existing.exists():
    ...
    def delete_dataset(self, dataset_id: int) -> bool:
        ...
        try:
            ...
            self.info_message.emit(
                f"Dataset '{dataset_name}' deleted ({objects_deleted} objects, {analyses_deleted} analyses)"
            )
            return True
    ...
    def run_analysis(
        ...
        **kwargs,
        ...
        try:
            ...
            if analysis_type.upper() == "PCA":
                ...
                if manova_group_by is not None:
                    try:
                        ...
                        self.logger.info(
                            f"MANOVA result keys: {manova_result.keys() if isinstance(manova_result, dict) else 'Not a dict'}"
                        )
            ...
            analysis = self._persist_analysis_results(
    ...
    def _persist_analysis_results(
        ...
        """Create the MdAnalysis record for this run and serialize results to JSON.

        Resolves group-by indices to variable names, creates the MdAnalysis row,
        then best-effort serializes object info, raw/superimposed landmarks, and
        PCA/CVA/MANOVA results into the record's JSON columns. A serialization
        failure is logged and swallowed so the record still persists.

        Returns:
            The saved MdAnalysis instance.
        """
    ...
    def _run_cva(self, landmarks_data: list[list], params: dict[str, Any]) -> dict[str, Any]:
        ...
        self.logger.info(
            f"Landmarks data shape: {len(landmarks_data)}x{len(landmarks_data[0]) if landmarks_data else 0}"
        )
    ...
    def _run_manova(self, landmarks_data: list[list], params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"MANOVA returned: {type(manova_result)}, keys: {manova_result.keys() if isinstance(manova_result, dict) else 'N/A'}"
            )
            ...
            if "test_statistics" in manova_result:
                ...
                for stat in manova_result["test_statistics"]:
                    stat_name = stat["name"]
                    stat_values = [stat["value"], stat["df_num"], stat["df_den"], stat["f_statistic"], stat["p_value"]]
    ...
    def _validate_dataset_for_analysis_type(self, analysis_type: str) -> tuple[bool, str]:
        ...
        objects_with_landmarks = list(
            self.current_dataset.object_list.where(
                (MdModel.MdObject.landmark_str.is_null(False)) & (MdModel.MdObject.landmark_str != "")
            )
        )
    ...
    def _validate_dataset_for_general_analysis(self, dataset) -> bool:
        ...
        if len(objects_with_landmarks) < 5:
            show_warning(
                None,
                f"Dataset '{dataset.dataset_name}' has too few objects with landmarks ({len(objects_with_landmarks)}). At least 5 objects required for analysis.",
            )
            return False
