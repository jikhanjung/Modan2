...
## 3. changelog 를 번역 대상에서 제외

`changelog.po` 가 미번역 111 / fuzzy 18 이었다. 이번 작업 때문이 아니라
**devlog 269가 `changelog.rst` 를 CHANGELOG.md 전문 include로 바꾸면서** 생긴
격차다. 그전에는 손으로 유지하던 축약 미러였기 때문에 devlog 262의 "8개 카탈로그
모두 0" 이 성립했지만, 그 시점부터 릴리스 이력 전체가 번역 대상이 됐다.

**제외하기로 했다.** 과거 릴리스 노트의 한국어판을 유지하는 값어치가 낮고, 그냥
두면 릴리스마다 번역 부채가 늘어난다.

구현에서 주의할 점이 둘 있었다.

1. **`sphinx-intl update` 가 `.po` 를 다시 만든다.** 삭제만 하면 다음 갱신 때
   조용히 돌아온다. 그래서 지우는 대신 **`.gitignore` 에 넣어** 재생성돼도 커밋될
   수 없게 했다. 관례가 아니라 강제다.
2. **Sphinx는 `.po` 가 아니라 컴파일된 `.mo` 를 읽는다.** `.po` 를 지운 뒤에도
   한국어 제목이 그대로 나와서 확인해 보니 이전 빌드가 남긴 `changelog.mo` 가
   읽히고 있었다. `.mo` 는 `docs/manual/.gitignore` 가 이미 제외하는 빌드 산출물
   이므로 저장소에는 영향이 없지만, **검증 중에 "제외가 안 먹는다"로 오인하기
   쉬운 지점** 이다. 지우고 다시 빌드하니 의도대로 동작했다.

검증: 한국어 빌드에서 `changelog.html` 은 영문 원문(`Changelog`, `A housekeeping
beta...`)으로, `installation.html` 은 한국어(`설치`)로 렌더링된다. 절차는
`docs/manual/README.md` 에 적었고, 나머지 7개 카탈로그를 0으로 유지하는지 확인하는
한 줄짜리 점검 명령도 함께 넣었다.
...
없음. 이번 세션에서 열린 항목은 Windows 실물 설치 검증뿐이며, TODOs에 있다.
