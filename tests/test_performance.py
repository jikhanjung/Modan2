...
class TestMdStatisticsPerformance:
    ...
    def test_cva_performance_large_dataset(self):
        ...
        group1_data = [[i + np.random.randn() * 0.1 for i in range(n_variables)] for _ in range(group_size)]
        group2_data = [[i + 10 + np.random.randn() * 0.1 for i in range(n_variables)] for _ in range(group_size)]
...
class TestMemoryUsage:
    ...
    def test_memory_cleanup_after_large_operations(self, mock_database):
        ...
        for i in range(100):
            obj = mm.MdObject.create(object_name=f"Memory Object {i}", dataset=dataset)
        ...
        print(
            f"\nMemory usage - Initial: {initial_memory:.2f}MB, Peak: {peak_memory:.2f}MB, Final: {final_memory:.2f}MB"
        )
