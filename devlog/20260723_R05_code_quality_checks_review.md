...
### mypy 전면 확장 — components/ + dialogs/ (2026-07-24)

레포 전역 두 패턴을 정리해 **components/·dialogs/ 전체를 게이트에 편입**:
- **matplotlib FigureCanvas:** `import FigureCanvas as FigureCanvas`는 mypy 스텁이
  모르는 별칭. 실사용 파일(analysis_info, data_exploration, dataset_analysis)은
  런타임 동일 클래스 `FigureCanvasQTAgg as FigureCanvas`로 교체, **미사용(죽은)
  import은 제거**(포맷 리더 등 ~10개 파일 복붙 잔재 — ruff F401 자동 정리).
- **OpenGL glut 조건부 import:** `glut = None` 후 재import를 mypy가 no-redef로
  오탐 → 의도된 fallback이므로 import 줄에 `# type: ignore[no-redef]`(14개 파일).
- **파일별 잔여 5개:** `Qt.WaitCursor`/`Qt.UserRole`을 스코프 열거형
  (`Qt.CursorShape.WaitCursor`, `Qt.ItemDataRole.UserRole`)으로, mouseReleaseEvent
  인자 주석 제거(형제 핸들러 관례), **`object_viewer_2d.draw_dataset` 중복 정의의
  죽은 첫 번째 제거**(Python은 두 번째만 실행 — R01류 twin/dead 버그 실제 수정).

→ **mypy 게이트: 11개 → 48개 소스**(core + components/ 전체 + dialogs/ 전체).
ruff·format 클린, 영향 테스트 374개 통과.

### 파서 퍼즈 확대 — 실제 버그 발견 (2026-07-24)

item 9(퍼즈)를 포맷 리더로 확대(`tests/test_fuzz_parsers.py`): TPS/NTS/X1Y1/
Morphologika + MdUtils read_tps/read_nts에 무작위 입력. 계약 = 제어된 에러
(ValueError/OSError/UnicodeError)만 나거나 정상 반환, 추한 크래시/행 없음.

**Morphologika에서 실제 버그 발견·수정:** 입력 `[`(섹션명 없는 대괄호)에서
