...
- **"Snap to curve" 체크박스**(`cbxSnapToCurve`)를 옵션 체크박스 열에 추가.
  **기본 켜짐**이며 **곡선 모드에서만 활성화**(다른 모드에선 비활성/회색). 곡선 모드
  진입 시 체크 상태를 뷰어에 적용(`_apply_snap`)하므로 커브 모드에 들어가면 스냅이
  바로 켜진다. 재진입해도 마지막 선택 복원.
...
#### 스냅 곡선 편집 버그 수정 (앵커 삽입 일관성 + 드래그 실시간)
초기 편집 구현의 두 결함을 수정:
- **삽입이 될 때만 됨**: 세그먼트 히트테스트가 희소 앵커 사이 직선(chord) 기준이라,
  보이는 조밀 곡선을 클릭해도 직선과 멀면 삽입 실패. 수정: 스냅 곡선은 **조밀
  경로(보이는 선) 근접**으로 판정하고, 새 앵커를 조밀 경로상 위치(최근접 정점
  기준 호길이 순서)로 앵커 목록에 삽입(`_insert_editpoint_near`,
  `_anchor_insert_index`). 손으로 그린 곡선은 기존 raw 세그먼트 히트테스트 유지.
- **드래그 시 곡선 실시간 미반영**: 재스냅이 release에서만 일어남. 수정:
  `mouseMove`에서 인접 세그먼트를 실시간 재스냅. 세그먼트당 ~52ms라 매 이동
  전체 재계산은 버벅여서, (1) LiveWire에 **다중 seed 예측맵 캐시**(`_pred_cache`,
  cap 16)를 추가하고 (2) `_resnap_selected_curve(moving_index)`가 **고정 이웃
